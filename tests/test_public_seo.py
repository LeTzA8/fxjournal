def test_landing_page_includes_canonical_and_search_metadata(client):
    ...
    assert b"<title>MyFXJournal | Forex Trading Journal With Weekly AI Review</title>" in response.data
...
def test_free_mt5_sync_page_has_indexable_metadata(client):
    ...
    assert response.status_code == 200
    assert b"Free MT5 Sync During Open Beta | MyFXJournal" in response.data
