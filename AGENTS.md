...
- For non-trivial tasks, choose one primary role from `/context/ROLES.md`; add secondary roles when the task clearly spans concerns
- Let the primary role drive the work; use secondary roles as additional review lenses, not replacements
- State `Role: <primary>` or `Role: <primary> + <secondary>` before acting
