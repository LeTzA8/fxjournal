...
| `manual VM scripts/` | Operator-only helpers (`gitpull.bat`, `monitor.bat`, `run_mt5_broker_seeding.bat`, `mt5_broker_seeding/`, Task Scheduler XML: `FX Journal MT5 * Watchdog.xml` vs `FX Journal MT5 * Worker Direct.xml`) | Not run automatically — document schedules yourself |
